...
const EmailItem = ({ email }) => {
    ...
    return (
        ...
        >
            {/* Checkbox (Mock) */}
            <div className="text-text-muted group-hover:text-text-secondary opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
            ...
            <button
                ...
                className={`transition-colors -ml-5 group-hover:ml-0 ${email.isStarred ? 'text-accent-yellow opacity-100 ml-0' : 'text-text-muted opacity-0 group-hover:opacity-100 hover:text-text-secondary'}`}
            >
